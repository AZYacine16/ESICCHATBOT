...
export function findBestAnswer(normalizedQuestion: string) {
  ...
  for (const row of rows as any[]) {
    const k = normalize((row as any).key);
    if (!k) continue;
  ...
  for (const row of rows as any[]) {
    const kNorm = normalize((row as any).key);
    if (!kNorm) continue;
    ...
    if (score > best.score) best = { score, value: (row as any).value };
  }
...
export function authenticateUser(username: string, password: string) {
  ...
  const user = db
    ...
    .get(trimmedUsername);
  if (!user) return null;
  const ok = bcrypt.compareSync(password, (user as any).password_hash);
  if (!ok) return null;
  return user;
}
